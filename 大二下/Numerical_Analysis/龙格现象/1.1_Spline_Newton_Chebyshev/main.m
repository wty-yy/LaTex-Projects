function main
    ...
    disp("N10 = ")
    disp(y) % 输出N10插值结果
    plot(x, subs(y, x), 'color', '#FBC02D', 'linewidth', 1); % N10图像
    hold on
    % Chebyshev插值10个插值点与平均插值点作比较
    xx = cos((1:2:21)*pi/22)';
    y = Newton(xx, f(xx));
    disp("T10 = ")
    disp(y) % 输出T10的结果
    plot(x, subs(y, x), 'color', '#4CAF50', 'linewidth', 1); % T10图像
    hold on
    % Chebyshev插值20个插值点与平均插值点作比较
    xx = cos((1:2:41)*pi/42)';
    y = Newton(xx, f(xx));
    disp("T20 = ")
    disp(y) % 输出T20的结果
    plot(x, subs(y, x), 'color', '#1E88E5', 'linewidth', 1); % T20图像
    hold on
    legend(["f(x)" "N10" "T10" "T20"]);
